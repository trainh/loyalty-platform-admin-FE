...
const columns = [
  ...
  { field: 'redeemed', headerName: 'Redeemed', width: 100 },
...
export default function Reward() {
  ...
  useEffect(() => {
    ...
    axios
      .get('http://13.232.213.53/api/v1/rewards')
      .then((response) => {
  ...
  function applySortFilter(array, query) {
    if (query) {
      return filter(array, (reward) => reward.name.toLowerCase().includes(query.toLowerCase()));
    }
    return rewards;
  }
  ...
  const filteredRewards = applySortFilter(rewards, filterName);
